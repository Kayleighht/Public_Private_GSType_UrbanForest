...
Public5.final<- Public5 + geom_col(alpha= 0.7, width= 0.7) + theme_classic() + 
                abundance.labs + scale_fill_manual(values = colour.table.publicab$Colour) +theme(legend.position = "none", axis.title = element_text(face= "bold", size= 12))
Public5.final
...
Private5.final<-Private5 + geom_col(alpha= 0.7, width = 0.7) + theme_classic()+ 
                abundance.labs + scale_fill_manual(values= colour.table.privateab$Colour) + theme(legend.position = "none", axis.title = element_text(face= "bold", size= 12))
Private5.final

##Arrange public and private plots into one figures
Public.Private5<-ggarrange(Public5.final, Private5.final,
                            labels = c("Public", "Private"), font.label = list(size= 12), hjust = -9.5)
Public.Private5
