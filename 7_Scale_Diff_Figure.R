#### Load Packages
Packages.7 <- c("plyr", "ggplot2", "tidyr","ggthemes", "dplyr")
...
site.scale.bar+ geom_bar(stat = 'identity', position = position_dodge(width=0.9)) + 
  ...
  scale.labs + theme_classic() + scale_fill_manual(values= colour.table.site$Colour) + theme(axis.title = element_text(face= "bold", size= 14))
...
parcel.scale.bar + geom_bar(stat = 'identity') + 
  ...
  theme_classic() + parcel.labs + scale_fill_manual(values= colour.table.parcel$Colour)+ theme(axis.title = element_text(face= "bold", size= 14))
